/* Modified ESP32 BMP280 + MPU6050 Web Server
   - Debounced IR handling to prevent LCD flicker
   - Optionally uses INPUT_PULLUP for IR pin (change if your wiring demands plain INPUT)
*/
...
// Custom library designed for the CRAM mechanism
#include "cram.h"
...
const char *NODE_B_IP = "192.168.31.187";
// ---------------------------------
...
/* ---------------- Security State ---------------- */
bool lastAuthStatus = false;
String lastAuthMessage = "Idle";

/* ---------------- Replay Protection ---------------- */
String nonceHistoryA[10];
int nonceIndexA = 0;
...
<body>
  ...
  <div>
  <button onclick="triggerAuth()">Authenticate Node B</button>
  <span id="auth_status"></span>
  </div>
<script>
const FLASK_PREDICT_URL = "http://172.29.150.145:5000/predict";
...
function calibrate() {
  ...
  fetch('/auth')
    .then(res => res.json())
    .then(d => {
      document.getElementById('auth_status').innerText = d.status;
    })
    .catch(() => {
      document.getElementById('auth_status').innerText = "Error";
    });
}
...
bool isNonceReusedA(String nonceStr) {
  for (int i = 0; i < 10; i++) {
    if (nonceHistoryA[i] == nonceStr) {
      return true;
    }
  }

  nonceHistoryA[nonceIndexA] = nonceStr;
  nonceIndexA = (nonceIndexA + 1) % 10;

  return false;
}

// -------------- CRAM Authentication Function ----------
bool performCRAMHandshake() {
  ...
  // 🔥 Replay Detection
  if (isNonceReusedA(nonceStr)) {
    Serial.println("⚠️ REPLAY ATTACK DETECTED (Node A)");
    lastAuthStatus = false;
    lastAuthMessage = "Replay Detected";
    return false;
  }

  HTTPClient http;
  String url = "http://" + String(NODE_B_IP) + "/cram?nonce=" + nonceStr;

  Serial.println("Sending Request:");
  Serial.println(url);

  http.begin(url);
  http.setAuthorization("Mokshagna", "mokshagna@3");
  ...
  if (httpCode == 200) {
    ...
    if (verify_hmac(nonce, 8, received_hmac)) {
      Serial.println("CRAM AUTH SUCCESS ✅");
      lastAuthStatus = true;
      lastAuthMessage = "Auth Success";
      http.end();
      return true;
    } else {
      Serial.println("CRAM AUTH FAILED ❌");
      lastAuthStatus = false;
      lastAuthMessage = "Auth Failed";
    }
  }
...
// -------- CRAM Challenge Handler -----------
void handleCRAMChallengeA() {
  ...
  // 🔥 Replay Detection (Server-side)
  if (isNonceReusedA(nonceStr)) {
    Serial.println("⚠️ REPLAY ATTACK DETECTED (Node A - Server)");
    lastAuthStatus = false;
    lastAuthMessage = "Replay Detected";
...
bool lastIrActive = false; // last known stable state written to LCD
bool irActive = false;     // current stable state (after debounce)

// ---- Debounce helper state ----
bool irRawLast = false;                // last raw read (immediate GPIO)
unsigned long irLastBounceMillis = 0;  // last time raw state changed
const unsigned long IR_DEBOUNCE_MS = 200; // hold new state for 200ms before accepting
...
void restoreNormalDisplay() {
  ...
  // Prevent an immediate periodic update from clobbering / re-triggering — update lcd_last
  lcd_last = millis();
}
...
void handleAuthTrigger() {
  ...
  bool success = performCRAMHandshake();  // modify function to return bool
...
void setup() {
  ...
  // Initialising the CRAM
  cram_init();

  // Use INPUT_PULLUP to avoid floating pin if your IR sensor outputs LOW when triggered.
  // If your wiring already provides pull-up/down or sensor expects plain INPUT, change to INPUT.
  ...
  server.on("/auth", handleAuthTrigger);
  server.on("/cram", handleCRAMChallengeA);
  server.on("/status", handleStatus);
  server.begin();
  Serial.println("HTTP server started.");

  // initialize raw and stable IR state to current reading
  bool raw = (digitalRead(IR_PIN) == LOW); // sensor logic: LOW means detected in your design
  irRawLast = raw;
  irLastBounceMillis = millis();
  // Do not "force" the LCD here; treat initial stable state as lastIrActive
  irActive = raw;
...
void loop() {
  ...
  // ---- IR raw read + debounce logic ----
  bool raw = (digitalRead(IR_PIN) == LOW); // LOW means detected per your wiring
  unsigned long now = millis();

  if (raw != irRawLast) {
    // raw changed — mark bounce time but don't flip stable state yet
    irRawLast = raw;
    irLastBounceMillis = now;
  } else {
    // raw same as last raw; if it's been stable longer than debounce window, accept it
    if ((now - irLastBounceMillis) >= IR_DEBOUNCE_MS) {
      // raw has been stable for the debounce period; consider it the new stable state
      if (raw != irActive) {
        // stable state changed
        irActive = raw;
        if (irActive != lastIrActive) {
          // only act on transitions from lastIrActive -> new stable irActive
          if (irActive) {
            // show alert once
            showIrAlert();
